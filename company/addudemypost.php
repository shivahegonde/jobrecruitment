...
if(isset($_POST)) {
	...
	$stmt = $conn->prepare("INSERT INTO udemy_post( title, description,udemylink,coursetype) VALUES (?,?,?,?)");

	$stmt->bind_param("ssss", $udemytitle, $description,$udemylink,$coursetype);

	$udemytitle = mysqli_real_escape_string($conn, $_POST['title']);
	$description = mysqli_real_escape_string($conn, $_POST['description']);
	$coursetype = mysqli_real_escape_string($conn, $_POST['coursetype']);
	
	$udemylink = mysqli_real_escape_string($conn, $_POST['udemylink']);
	$sql = "INSERT INTO demoudemy(title,description,udemylink,coursetype) VALUES ('$udemytitle', '$description','$udemylink','$coursetype')";
	//$conn->query($sql);
	...
	if($stmt->execute()) {
		...
		header("Location: jobrecruitment.php");
		exit();
	} else {
		//If data failed to insert then show that error. Note: This condition should not come unless we as a developer make mistake or someone tries to hack their way in and mess up :D
		echo "Error ".$udemytitle;
	}
	...
	header("Location: create-udemy-post.php");
	exit();
}
